...
try {
    ...
    if (!is_dir($uploads_dir)) {
        if (!mkdir($uploads_dir, 0755, true)) throw new Exception('Could not create uploads/updates/ directory.');
    }
    ...
    if ($update_id) {
        ...
        if (!empty($new_image_paths)) {
            $max_res = $conn->query("SELECT COALESCE(MAX(sort_order),0)+1 AS nxt FROM update_images WHERE update_id=$update_id");
            ...
            foreach ($new_image_paths as $pi => $path) { $ord = $nxt + $pi; $ins->bind_param("isi", $update_id, $path, $ord); $ins->execute(); }
            $ins->close();
        }

        $img_note = !empty($new_image_paths) ? ' ' . count($new_image_paths) . ' new image(s) added.' : '';
        setAlert('Post "' . $title . '" updated successfully.' . $img_note, 'success');
    } else {
        ...
        if (!empty($new_image_paths)) {
            $ins = $conn->prepare("INSERT INTO update_images (update_id, image_path, sort_order) VALUES (?,?,?)");
            foreach ($new_image_paths as $pi => $path) { $ins->bind_param("isi", $new_id, $path, $pi); $ins->execute(); }
            $ins->close();
        }

        $img_note = !empty($new_image_paths) ? ' ' . count($new_image_paths) . ' image(s) uploaded.' : '';
        setAlert('Post "' . $title . '" created successfully.' . $img_note, 'success');
    }
