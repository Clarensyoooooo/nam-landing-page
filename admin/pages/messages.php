...
<div class="admin-card">
    <?php if (!empty($messages)): ?>
        <table class="admin-table" id="messagesTable">
            <thead>
                <tr>
                    <th>Name</th><th>Email</th><th>Phone</th><th>Service</th><th>Date</th><th>Status</th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
                ...
                ?>
                <tr data-status="<?php echo $sk; ?>"
                    data-search="<?php echo strtolower(htmlspecialchars($message['full_name'].' '.$message['email'].' '.($message['phone']??'').' '.($message['service_needed']??'').' '.$message['message'])); ?>">
...
<div class="confirm-modal-overlay" id="msgDeleteConfirmModal">
    <div class="confirm-modal-box">
        <div class="confirm-modal-header">
            ...
            <div><h3>Delete Message</h3><p>This action cannot be undone</p></div>
        </div>
            ...
            document.querySelectorAll('#messagesTable tbody tr').forEach(function(row) {
                if (row.querySelector('[onclick="viewMessage('+id+')"]')) {
                    ...
                    if (badge && badge.classList.contains('unread')) {
                        badge.className='status-badge read'; badge.innerHTML='<i class="fas fa-check"></i> Read';
